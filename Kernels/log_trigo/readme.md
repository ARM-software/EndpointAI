...
The code is compiled with arm llvm-v18.1.3, optimization -o1. The cycle-count is measured on a Cortex-M85 development board (FPB-RA8E1). In practice the user will find lower numbers because the subroutines are very small, the code runs from Flash+cache instead of TCM. When inlined with the other parts of the application codes, the compiler will interleave other instructions to remove any pipeline stalls. Additionally when a program calls two operations like sin(x) and cos(x) there are common constants and table pointer initializations which will be merged. Conclusion, the numbers are conservative.
