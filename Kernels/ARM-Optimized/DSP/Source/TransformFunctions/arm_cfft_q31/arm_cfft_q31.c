...
#define RAD2_BFLY_STG1_FX_MVE(sz, conj)                                       \
    ".p2align 2                                                            \n"\
    "   wls                 lr, %[count], 1f                               \n"\
    "   vldrw.32            q0, [%[pIn0]]                                  \n"\
    "2:                                                                    \n"\
    ...
    "   vhsub.s"#sz"        q3, q0, q1                                     \n"\
    "   vldrw.32            q0, [%[pIn0], #16]                             \n"\
    cmplx_fx_mul_r_##conj(sz, q4, q3, q7)                                     \
    "   vstrw.32            q2, [%[pIn0]], 16                              \n"\
    cmplx_fx_mul_i_##conj(sz, q4, q3, q7)                                     \
    "   vstrw.32            q4, [%[pIn1]], 16                              \n"\
    "   le                  lr, 2b                                         \n"\
    "1:                                                                    \n"
...
void arm_bitreversal_32_inpl_mve_asm(
        uint32_t *pSrc,
...
#ifdef USE_ASM
    __asm volatile(
        RAD2_BFLY_STG1_FX_MVE(32, conj)
        : [pIn0] "+r"(pIn0), [pIn1] "+r"(pIn1)
        : [count] "r"(n2 / 2), [pCoef] "r"(pCoef)
        : "lr", "memory",
          "q0", "q1", "q2", "q3", "q4", "q7");
#else
...
#ifdef USE_ASM
    __asm volatile(RAD2_BFLY_STG1_FX_MVE(32, /*no conjugate*/)
        : [pIn0] "+r"(pIn0)
          , [pIn1] "+r"(pIn1)
        : [count] "r"(n2 / 2), [pCoef] "r"(pCoef)
        : "lr", "memory",
