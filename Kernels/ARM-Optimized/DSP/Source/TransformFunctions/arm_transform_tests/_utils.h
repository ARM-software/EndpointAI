...
#define SETUP_SINGLE_VECTORS_CMPLX(TYP, size, dstOffs)                                          \
        ...
        ALIGN16 TYP dstScal[CMPLX_DIM * MAX_FFT_LENGTH + 2*EXTRA_TAIL_CHK] = {0}, *pdstScal;    \
        ALIGN16 TYP dstVec[CMPLX_DIM*MAX_FFT_LENGTH + 2*EXTRA_TAIL_CHK] = {0}, *pdstVec;        \
        pA = A;                                                                                 \
...
#define RUN_Scal_VS_Vec_FFT_WITH_FLAGS(fct, thresh,ifftFlag,bitReverseFlag,format)             \
	int64_t         ref, opt;                                                                  \
