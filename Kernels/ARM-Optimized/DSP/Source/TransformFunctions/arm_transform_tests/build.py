...
def project_name(config):
    return f"unittest_{config.project}.{config.optimize}+{config.device[2]}"
...
def build(config, results):
    ...
    src = Path(f"unittest.{config.compiler[0].lower()}-cdefault.yml")
    dst = Path("unittest.cdefault.yml")
...
def csolution(project):
    return ["csolution", "convert", "-s", "unittest.csolution.yml", "-c", project]
